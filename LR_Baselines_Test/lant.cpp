...
int lant::find_aprx(double** half_hessi, double &tuc_lam) {
	...
	cblas_dscal(this->cate*this->fea_num, APRX_RAN_SIGMA, tmp_halhessi[0], 1);
	...
	int rank_true;
	for (int i=0;i<this->fea_rank+OVER_SAMP;i++)
		if (tmp_sigma[i] < REG_EPS) {
			rank_true = i;
			break;
		}
	rank_true--;
	tuc_lam = tmp_sigma[rank_true];

	tmp_ure = (double**)malloc(sizeof(double*)*(this->fea_rank + OVER_SAMP));
	tmp_ure[0] = (double*)malloc(sizeof(double)*(this->fea_rank + OVER_SAMP)*rank_true);
	for (int i = 1; i < this->fea_rank + OVER_SAMP; i++)
		tmp_ure[i] = tmp_ure[0] + i*rank_true;
	for (int i = 0; i < rank_true; i++) {
		cblas_dcopy(this->fea_rank + OVER_SAMP, &tmp_u[0][i], this->fea_rank + OVER_SAMP, &tmp_ure[0][i], rank_true);
		cblas_dscal(this->fea_rank + OVER_SAMP, 1 / (sqrt(tmp_sigma[i])), &tmp_ure[0][i], rank_true);
	}
		

	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, this->cate*this->fea_num, rank_true, this->fea_rank + OVER_SAMP, 1.0, tmp_halhessi[0], this->fea_rank + OVER_SAMP, tmp_ure[0], rank_true, 0, half_hessi[0], rank_true);
